...
import time
from CHANDU_Banking.com.service.validations import *
from CHANDU_Banking.com.service.mail_operations import *
from CHANDU_Banking.com.service.user_security import *
...
class CreateAccount:
    def accountCreation(self):
        ...
        insert_data('chandu_users', first_name=self.fname, last_name=self.lname, email=self.email, dob=self.dob,
                    salt=self.salt, password=self.password)


        # fetching userId of the new user created
        self.userId = fetch_data('chandu_users', email=self.email)

        # inserting account info into database, table flm_accounts for new user created
        insert_data('chandu_accounts', user_id=self.userId[0][0], account_number=self.accountNumber, balance=self.initialDeposit, is_active=True)
...
def login(user_mail):
    ...
    while True:
        ...
        if user_mail in fetch_column_data('chandu_users', 'email'):
            user_password = input("Password: ")
            print()

            # fetching user data from db using user_mail
            db_data = fetch_data('chandu_users', email=user_mail)
            ...
            if failedLoginAttemps > MAX_LOGIN_ATTEMPTS:
                print("[-] Maximum Login Attempts Reached.. Account Locked... "
                      "to unlock contact chandubank@gmail.com")
                print()
                ...
                if hashed_user_password == db_password:
                    ...
                    update_data('chandu_users', values={'failedcountattempts': 0}, conditions={'email': user_mail})
    ...
    try:
        ...
        if user_dob == str(db_dob):
            ...
            if mailOtpVerification(user_mail):
                ...
                update_data('chandu_users', values={'password': updated_password,'salt': updated_salt}, conditions={'email': user_mail})
