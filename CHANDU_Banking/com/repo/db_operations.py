...
def insert_data(table_name, **kwargs):
    ...
    try:
        ...
        if table_name == "chandu_users":
            sql = ("INSERT INTO chandu_users (first_name, last_name, email, dob, salt, password) "
                   "VALUES (%(first_name)s, %(last_name)s, %(email)s, %(dob)s, %(salt)s, %(password)s)")

        # inserting data into flm_accounts table
        elif table_name == "chandu_accounts":
            sql = ("INSERT INTO chandu_accounts (user_id, account_number, balance, is_active) "
                   "VALUES (%(user_id)s, %(account_number)s, %(balance)s, %(is_active)s)")

        # inserting data into flm_transactions table
        elif table_name == "chandu_transactions":
            sql = (
                "INSERT INTO chandu_transactions (user_id, account_id, amount, from_account, to_account, trans_date, trans_type) "
...
# insert_data('chandu_transactions', user_id=1, account_id=1, amount=200.00, from_account='1234567890', to_account='0987654321', date='2023-09-19', trans_type='db')
#accounts = fetch_data('chandu_accounts', user_id=1)
#users = fetch_column_data('chandu_users','email')
# accounts = fetch_data('chandu_users', email='example@gmail.com')
# transactions = fetch_data('chandu_transactions', user_id=1, account_id=1)
# print(accounts)
# print(accounts[0][8])
# print(type(accounts[0][8]))
# users = fetch_data('chandu_users', email='johndoe@example.com')
#print(users)
# print(users[0][4])
# print(accounts)
# print(transactions)
...
# update_data('chandu_users', values={'last_name': 'Smith'}, conditions={'email': 'johndoe@example.com'})
# update_data('chandu_accounts', values={'balance': 1200.00}, conditions={'account_number': '1234567890'})
# update_data('chandu_transactions', values={'amount': 250.00}, conditions={'trans_id': 1})